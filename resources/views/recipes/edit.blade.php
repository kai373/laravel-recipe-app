<x-app-layout>
    ...
    <form action="{{ route('recipe.update', ['id' => $recipe['id']]) }}" method="POST"
        class="w-10/12 p-4 mx-auto bg-white rounded" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
        <!-- クロスサイトリクエストフォージェリ -->
        ...
        <div class="grid grid-cols-2 rounded border border-gray-500 my-4">
            <div class="col-span-1">
                <img id="preview" class="object-cover w-full aspect-video" src="{{ $recipe['image'] }}"
                    alt="recipe-image">
            ...
            <div class="col-span-1 p-4">
                ...
                <select name="category_id" class="border border-gray-300 p-2 mb-4 w-full rounded">
                    ...
                    @foreach ($categories as $c)
                        <option value="{{ $c['id'] }}"
                            {{ ($recipe['category_id'] ?? null) == $c['id'] ? 'selected' : '' }}>{{ $c['name'] }}
                        </option>
                    @endforeach
                </select>
                    ...
                    @foreach ($recipe['ingredients'] as $i => $oi)
                        <div class="ingredient flex items-center mb-4">
                            ...
                            <input type="text" value="{{ $oi['quantity'] }}"
                                name="ingredients[{{ $i }}][quantity]" placeholder="分量"
        ...
        <div class="flex justify-center">
            <button type="submit"
                class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">レシピを更新する</button>
        </div>
        <!-- underline  -->
        <hr class="my-4">
        ...
        <div id="steps">
            @foreach ($recipe['steps'] as $i => $os)
                <div class="step flex justify-between items-center mb-2">
                    ...
                    <input type="text" value="{{ $os['description'] }}" name="steps[]" placeholder="手順を入力"
                        class="border border-gray-300 p-2 w-full rounded">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                        class="w-10 h-10 ml-4 step-delete text-gray-600">
                        <path fill-rule="evenodd"
                            d="M16.5 4.478v.227a48.816 48.816 0 013.878.512.75.75 0 11-.256 1.478l-.209-.035-1.005 13.07a3 3 0 01-2.991 2.77H8.084a3 3 0 01-2.991-2.77L4.087 6.66l-.209.035a.75.75 0 01-.256-1.478A48.567 48.567 0 017.5 4.705v-.227c0-1.564 1.213-2.9 2.816-2.951a52.662 52.662 0 013.369 0c1.603.051 2.815 1.387 2.815 2.951zm-6.136-1.452a51.196 51.196 0 013.273 0C14.39 3.05 15 3.684 15 4.478v.113a49.488 49.488 0 00-6 0v-.113c0-.794.609-1.428 1.364-1.452zm-.355 5.945a.75.75 0 10-1.5.058l.347 9a.75.75 0 101.499-.058l-.346-9zm5.48.058a.75.75 0 10-1.498-.058l-.347 9a.75.75 0 001.5.058l.345-9z"
                            clip-rule="evenodd" />
                    </svg>
    ...
    <form action="{{ route('recipe.destroy', ['id' => $recipe['id']]) }}" class="w-10/12 my-6 mx-auto" method="POST">
        ...
        <button id="delete" type="submit"
            class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">レシピを削除する</button>
    </form>
</x-app-layout>
